...
export default async (req) => {
  ...
  if (!key) return Response.json({ ok:false, error:"key가 ?�습?�다." }, { status:400 });

  const store = getStore("disolveworks-uploads");
  const entry = await store.getMetadata(key);
  if (!entry) return Response.json({ ok:false, error:"?�일??찾을 ???�습?�다." }, { status:404 });

  const m = entry.metadata || {};
  const partNum = Math.max(1, Math.min(12, parseInt(body.partNum ?? m.partNum,10) || 1));
  const sceneRaw = cleanText(body.sceneRaw ?? m.sceneRaw ?? m.scene, 20).replace(/??/,"").replace(/[^0-9A-Za-z가-??_-]/g,"");
  const updated = {
    ...m,
    partNum,
    part:`${partNum}부`,
    sceneRaw,
    scene: sceneRaw ? `${sceneRaw}?? : m.scene,
  ...
  if (blob === null) return Response.json({ ok:false, error:"?�일 본문??찾을 ???�습?�다." }, { status:404 });
